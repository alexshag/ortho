...
{
	return PI*0.5*(1.0 - e2*( 1.0/4.0 + e2*( 3.0/64.0 + e2*(5.0/256.0) )));
}
...
int geo_direct_wgs84(double lat1, double lon1, double az1, double s, double *lat2, double *lon2, double *az2 )
{
	...
	{	
		...
		*az2 = 180.0 + az1;
		if( *az2 > 360.0 ) *az2 -= 360.0;
			return 0;
	} else if( MIN_VALUE < fabs(cosphi1) ) 
	{	
		/*
		* non-polar origin
	...
	{			
		...
		double paz = ( phi1 < 0.0 ? 180.0 : 0.0 );
			double zero = 0.0f;
		return geo_direct_wgs84( zero, lon1, paz, dM, lat2, lon2, az2 );
	} 
}
...
int geo_inverse_wgs84(double lat1, double lon1, double lat2, double lon2, double *az1, double *az2, double *s)
{
	...
	} else if(  fabs(cosphi1) < testv ) 
	{
		...
		int k = geo_inverse_wgs84( lat2,lon2,lat1,lon1, az1,az2,s );
		SG_UNUSED(k);
	...
	} else if( fabs(cosphi2) < testv ) 
	{
		...
		int k = geo_inverse_wgs84( lat1, lon1, lat1, _lon1, az1, az2, s );
		SG_UNUSED(k);
		...
		*az2 = *az1 + 180.0;
		if( *az2 > 360.0 ) *az2 -= 360.0; 
		return 0;
	} else if( (fabs( fabs(lon1-lon2) - 180 ) < testv) && (fabs(lat1+lat2) < testv) ) 
	{
		/* Geodesic passes through the pole (antipodal) */
		double s1,s2;
		geo_inverse_wgs84( lat1,lon1, lat1,lon2, az1,az2, &s1 );
		geo_inverse_wgs84( lat2,lon2, lat1,lon2, az1,az2, &s2 );
		*az2 = *az1;
		*s = s1 + s2;
		return 0;
	} else 
	{
		...
		do 
		{
			...
			(sig+tc*sinsig*(c2sigm+tc*cossig*(-1.0+2.0*c2sigm*c2sigm)));
