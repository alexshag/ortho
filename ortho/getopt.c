...
/* Scan elements of ARGV (whose wplist_size is ARGC) for option characters
given in OPTSTRING.
		...
		switch (c)
		{
			...
			case '9':
				if (digit_optind != 0 && digit_optind != this_option_optind)
					_tprintf("digits occur in two different argv-elements.\n");
			...
			case 'c':
				_tprintf("option c with value `%s'\n", optarg);
				break;
			...
			default:
				_tprintf("?? getopt returned character code 0%o ??\n", c);
		}
	}

	if (optind < argc)
	{
		_tprintf("non-option ARGV-elements: ");
		while (optind < argc)
			_tprintf("%s ", argv[optind++]);
		_tprintf("\n");
	}
